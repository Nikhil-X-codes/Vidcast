...
const SearchComponent = () => {
  ...
  const searchHoverShadow = theme === 'dark' 
    ? 'hover:shadow-gray-800/50' 
    : 'hover:shadow-gray-300/50';

  const searchIconColor = theme === 'dark' 
    ? 'text-gray-400' 
    : 'text-gray-500';

  const focusedIconColor = theme === 'dark' 
    ? 'text-pink-400' 
    : 'text-pink-500';
  ...
  const clearButtonColor = theme === 'dark' 
  ...
  return (
    <div className={`animate-fadeIn px-4 ${bgColor} min-h-screen`}>
      <motion.div 
      ...
      >
        <form onSubmit={handleSearch} className="relative">
          <div className={`relative flex items-center rounded-full transition-all duration-300 ${searchBg} border ${searchHoverShadow} px-6 py-3 shadow-lg ${isFocused ? 'ring-2 ring-pink-500/50' : ''}`}>
            <div className="absolute inset-y-0 left-0 flex items-center pl-5 pointer-events-none">
              <Search className={`h-5 w-5 transition-colors ${searchIconColor} ${isFocused ? focusedIconColor : ''}`} />
            </div>
            <input
              type="text"
              value={searchQuery}
              ...
              className={`block w-full rounded-full bg-transparent py-1 pl-12 pr-10 ${textColor} ${placeholderColor} focus:outline-none text-lg`}
            />
            {searchQuery && (
              <button
                type="button"
                onClick={clearSearch}
                className={`absolute right-4 p-1 rounded-full transition-all ${clearButtonColor}`}
              >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            type="submit"
            className={`absolute right-2 top-1/2 transform -translate-y-1/2 px-4 py-1.5 rounded-full text-sm font-medium ${buttonBg} text-white shadow-md hidden sm:block`}
          >
            Search
          </motion.button>
        </form>
      ...
      {searchQuery && (
        ...
        >
          <h2 className={`text-2xl md:text-3xl font-bold mb-6 flex items-center ${textColor}`}>
          ...
          {loading ? (
            ...
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className={`text-center py-12 rounded-xl ${resultsBg}`}
            >
              ...
              <p className={`max-w-md mx-auto ${errorTextColor}`}>
                {error}
              </p>
              <button
                onClick={handleSearch}
                className={`mt-6 px-6 py-2 rounded-full font-medium transition-all ${buttonBg} text-white shadow-md`}
              >
                Try Again
              </button>
            </motion.div>
            ...
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className={`text-center py-16 rounded-xl ${noResultsBg}`}
            >
              <div className="inline-flex items-center justify-center w-20 h-20 mb-5 rounded-full bg-pink-100/50 text-pink-600">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <h3 className={`text-xl font-semibold mb-2 ${noResultsText}`}>
                No videos found
              </h3>
              <p className={`max-w-md mx-auto mb-6 ${errorTextColor}`}>
